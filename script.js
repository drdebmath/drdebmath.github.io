...
function displayResearchInterests(interests) {
  document.getElementById("research_interests_content").innerHTML = `
    <p class="bg-white dark:bg-gray-800 p-4 shadow rounded dark:text-white transition-colors duration-200">${
      interests ? interests.join(", ") : "No research interests listed."
    }</p>
  `;
}
...
function displayAllPublications(publications, groupBy = "year") {
  ...
  container.className =
    "publications-container flex flex-wrap gap-4 items-stretch mt-4";
  const colors = {
        ...
        <h3 class="group-title text-xl font-bold mb-4 dark:text-white transition-colors duration-200">${
          groupBy === "type"
            ? group.type.charAt(0).toUpperCase() +
              group.type.slice(1).toLowerCase() +
              "s"
            : group.year
        }</h3>
  ...
  list.innerHTML = talks
    ...
    <li class="talk-item mb-4">
        <div class="bg-white dark:bg-gray-800 p-4 shadow rounded transition-colors duration-200">
            <p class="talk-title dark:text-white"><strong>Title:</strong> ${
              talk.title
            }</p>
            <p class="talk-event dark:text-white"><strong>Event:</strong> ${
              talk.event
            }</p>
            <div class="talk-links mt-2">
              ${
                talk.links
                  ? Object.entries(talk.links)
                      .map(
                        ([key, url]) =>
                          `<a href="${url}" class="text-blue-600 dark:text-blue-400 hover:underline transition-colors duration-200 mr-2">${
              ...
              ${
                talk.tweet
                  ? `<a href="${talk.tweet}" class="text-blue-600 dark:text-blue-400 hover:underline transition-colors duration-200 mr-2">Tweet</a>`
                  : ""
              }
              ${
                talk.author
                  ? `<strong class="talk-author-label dark:text-white">Author:</strong> <a href="${talk.author.url}" class="text-blue-600 dark:text-blue-400 hover:underline transition-colors duration-200">${talk.author.name}</a>`
                  : ""
              }
              ${
                talk.Place
                  ? `<strong class="talk-place-label dark:text-white">Place:</strong> <span class="talk-place dark:text-white">${talk.Place}</span>`
                  : ""
              }
            </div>
        </div>
...
function displaySkills(skills) {
  ...
  container.innerHTML = `
    <div class="skills-content bg-white dark:bg-gray-800 p-4 shadow rounded transition-colors duration-200">
      <p class="dark:text-white"><strong class="skills-label dark:text-white">Programming:</strong> ${
        skills.programming ? skills.programming.join(", ") : "Not listed"
      }</p>
      <p class="dark:text-white"><strong class="skills-label dark:text-white">Python Libraries:</strong> ${
        skills.python_libraries
  ...
  container.innerHTML = `
    <ul class="community-services-list list-disc pl-5">
      ...
      <li class="community-services-item mb-2 dark:text-white">
        ...
        <p class="community-services-text ml-4">${
          services.pc_member_for?.join(", ") || "Not listed"
        }</p>
      </li>
      <li class="community-services-item mb-2 dark:text-white">
        <strong class="community-services-label dark:text-white">Organizing committee:</strong>
        <p class="community-services-text ml-4">${
          services.organizing_committee?.join(", ") || "Not listed"
        }</p>
      </li>
    </ul>
  `;
